<div class="menu-item bg-white border border-gray-200 rounded-lg p-4 mb-2" data-menu-id="{{ $menu->id }}"
    data-title="{{ $menu->title }}" data-type="{{ $menu->type }}" data-url="{{ $menu->url }}"
    data-target="{{ $menu->target }}" data-icon="{{ $menu->icon }}" data-css-class="{{ $menu->css_class }}"
    data-active="{{ $menu->is_active ? '1' : '0' }}" data-parent-id="{{ $menu->parent_id }}"
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3">
            ...
            <div class="flex-1">
                <div class="flex items-center space-x-2">
                    ...
                    <!-- Status Badge -->
                    <span
                        class="px-2 py-1 text-xs rounded-full {{ $menu->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
        ...
        <div class="flex items-center space-x-2">
            ...
            @if($menu->final_url && $menu->final_url !== '#')
                <a href="{{ $menu->final_url }}" target="_blank" class="text-gray-400 hover:text-blue-600" title="Preview">
            ...
            <!-- Edit -->
            <button onclick="openEditModal({{ $menu->id }})" class="text-gray-400 hover:text-blue-600" title="Edit">
                <i class="fas fa-edit"></i>
            </button>

            <!-- Delete -->
            <button onclick="deleteMenu({{ $menu->id }})" class="text-gray-400 hover:text-red-600" title="Delete">
