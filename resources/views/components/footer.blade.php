{{-- Footer Component --}}
@php
    $quickLinks = \App\Models\NavigationMenu::getMenuTree('quicklink');
                ...
                <div class="flex items-center mb-6">
                    @if($siteSettings['logo'] ?? false)
                        <img src="{{ asset('storage/' . $siteSettings['logo']) }}"
                            alt="{{ $siteSettings['site_name'] ?? config('app.name') }}" class="h-10 w-auto mr-3">
                    @else
                ...
                <div class="flex space-x-4">
                    @if($siteSettings['social_facebook'] ?? false)
                        <a href="{{ $siteSettings['social_facebook'] }}"
                            class="text-gray-400 hover:text-white transition-colors" aria-label="Facebook" target="_blank">
                    ...
                    @if($siteSettings['social_twitter'] ?? false)
                        <a href="{{ $siteSettings['social_twitter'] }}"
                            class="text-gray-400 hover:text-white transition-colors" aria-label="Twitter" target="_blank">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                <path
                    ...
                    @if($siteSettings['social_instagram'] ?? false)
                        <a href="{{ $siteSettings['social_instagram'] }}"
                            class="text-gray-400 hover:text-white transition-colors" aria-label="Instagram" target="_blank">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                <path
                    ...
                    @if($siteSettings['social_linkedin'] ?? false)
                        <a href="{{ $siteSettings['social_linkedin'] }}"
                            class="text-gray-400 hover:text-white transition-colors" aria-label="LinkedIn" target="_blank">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                <path
                    ...
                    @if($siteSettings['social_youtube'] ?? false)
                        <a href="{{ $siteSettings['social_youtube'] }}"
                            class="text-gray-400 hover:text-white transition-colors" aria-label="YouTube" target="_blank">
                            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                                <path
                    ...
                    @foreach($quickLinks as $menu)
                        <li>
                            <a href="{{ $menu->final_url }}"
                                class="text-gray-300 hover:text-white transition-colors flex items-center {{ $menu->css_class }}"
        ...
        <div class="container mx-auto px-4 py-6">
            <div class="flex flex-col md:flex-row justify-between items-center">
                <div class="text-gray-400 text-sm mb-4 md:mb-0">
                    <p>&copy; {{ date('Y') }} {{ $siteSettings['site_name'] ?? config('app.name') }}. All rights
                        reserved.</p>
                </div>
                ...
                <div class="flex flex-wrap justify-center md:justify-end space-x-6 text-sm">
                    @foreach($bottomMenus as $menu)
                        <a href="{{ $menu->final_url }}"
                            class="text-gray-400 hover:text-white transition-colors {{ $menu->css_class }}"
                            @if($menu->target) target="{{ $menu->target }}" @endif>
                            {{ $menu->title }}
                        </a>
                    @endforeach
